...
function encodeString(string memory str) public pure returns (bytes memory) {
    ...
    function toBytes(uint256 x, uint256 len) private pure returns (bytes memory) {
...
function _computeRootHash(bytes32[] memory leaves) public view returns (bytes32) {
